...
class _AnalysisContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        // 01_收集：灵感分解（分阶段方法）
        if (analysis.stageId == '01_收集') ...[_buildInspirationSection(context)],

        // 其他阶段：结构分析
        if (analysis.stageId != '01_收集') ...[_buildStructureSection(context)],
      ],
    );
  }

  /// 01_收集：灵感片段区块（采纳 → 生成 02_分组 文件）
  Widget _buildInspirationSection(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        const _SectionTitle('灵感片段（可采纳为 02_分组）'),

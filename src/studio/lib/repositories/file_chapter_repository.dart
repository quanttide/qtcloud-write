...
/// 基于文件系统的章节数据访问实现。
///
/// 对齐 qtcloud-write CLI 的四命令产物目录（workdir 下）：
/// - 01_收集 → journal/（日志条目，YYYY-MM-DD.md）
/// - 02_分组 → groups/（主题分组，<主题>.md）
/// - 03_初稿 → materials/<主题>.md（不含定稿）
/// - 04_定稿 → materials/<主题>-定稿.md（以 -定稿 后缀区分）
class FileChapterRepository implements ChapterRepository {
  ...
  /// 阶段目录映射（03/04 共用 materials/，靠后缀区分）
  ...
  /// 定稿文件后缀
  static const String _finalSuffix = '-定稿.md';
  ...
  Future<List<Chapter>> getChapters() async {
    ...
    for (final entry in _stageDirectories.entries) {
      ...
      final stageDir = path.join(basePath, entry.value);
      final dir = Directory(stageDir);
      
      if (await dir.exists()) {
        await for (final entity in dir.list()) {
          if (entity is File && entity.path.endsWith('.md')) {
            // 03_初稿 排除定稿文件；04_定稿 仅保留定稿文件
            if (stageId == '03_初稿' && entity.path.endsWith(_finalSuffix)) {
              continue;
            }
            if (stageId == '04_定稿' && !entity.path.endsWith(_finalSuffix)) {
              continue;
            }
  ...
  Future<Chapter> createChapter(String stageId, String title) async {
    ...
    final dirPath = path.join(basePath, stageDir);
  ...
  Future<void> moveChapter(String id, String targetStageId) async {
    ...
    final targetDirPath = path.join(basePath, targetStageDir);
    ...
    // 目标文件名：移入 04_定稿 时追加 -定稿 后缀，移出时去除
    var targetName = path.basename(chapter.path);
    if (targetStageId == '04_定稿' && !targetName.endsWith(_finalSuffix)) {
      targetName = targetName.replaceAll('.md', '') + _finalSuffix;
    } else if (targetStageId != '04_定稿' && targetName.endsWith(_finalSuffix)) {
      targetName = targetName.replaceAll(_finalSuffix, '.md');
    }
    final targetPath = path.join(targetDirPath, targetName);
  ...
  String _parseTitleFromFileName(String fileName) {
    // 移除定稿后缀（04_定稿 的 <主题>-定稿）
    if (fileName.endsWith('-定稿')) {
      fileName = fileName.substring(0, fileName.length - '-定稿'.length);
    }
    // 移除编号前缀，如 "1_1_" -> ""
  ...
  int _countWords(String content) {
    ...
    final cleanContent = content
        ...
        .replaceAll(RegExp(r'```.*?```', multiLine: true), '')  // 代码块

...
///
/// API key 从运行时环境变量 DEEPSEEK_API_KEY 读取（桌面端）：
/// ```bash
/// DEEPSEEK_API_KEY=sk-xxx flutter run -d linux
/// ```
/// 可通过 QTCLOUD_WRITE_LLM_BASE_URL / QTCLOUD_WRITE_LLM_MODEL（环境变量或 dart-define）覆盖。
library;
...
class LLMConfig {
  ...
  /// API key 必须通过 DEEPSEEK_API_KEY 提供。
  factory LLMConfig.defaults() {
    const dartUrl = String.fromEnvironment('QTCLOUD_WRITE_LLM_BASE_URL');
    const dartModel = String.fromEnvironment('QTCLOUD_WRITE_LLM_MODEL');

    // 运行时环境变量（桌面端；Web 无环境变量，回退 dart-define）
    final apiKey = kIsWeb ? '' : (Platform.environment['DEEPSEEK_API_KEY'] ?? '');
    final envUrl = kIsWeb ? null : Platform.environment['QTCLOUD_WRITE_LLM_BASE_URL'];
    final envModel = kIsWeb ? null : Platform.environment['QTCLOUD_WRITE_LLM_MODEL'];
...
class LLMClient {
  ...
  /// [stageId] 决定分析方法：01_收集 → 灵感分解；其他阶段 → 结构分析。
  ...
  String _systemPrompt(String stageId) => stageId == '01_收集'
      ? '''
你是创作灵感分解助手。任务：把收集阶段的自由日志分解为多个灵感片段。
硬性规则：
1. 绝不改写、重写、润色原文的任何字符
2. 每个片段必须引用原文行号范围
...
''';
    ...
    return '''
工作流阶段（叙事工程）：01_收集（素材）/ 02_分组（主题）/ 03_初稿（成文）/ 04_定稿（定稿）。
已忽略的建议（不要再提出）：$ignored

...
void main() {
  ...
  setUp(() async {
    ...
    originalFile = File(path.join(tempDir.path, '03_初稿', '1_1_测试章节.md'));
  ...
  test('保存分析结果写入 .analysis/ 目录，原文不受影响', () async {
    ...
    final analysisFile = File(path.join(
        tempDir.path, '03_初稿', '.analysis', '1_1_测试章节.json'));
    ...
    expect(loaded.suggestedStageId, '04_定稿');
  ...
  test('删除 .analysis/ 目录不影响原文（回到纯文本时代）', () async {
    ...
    final analysisDir = Directory(
        path.join(tempDir.path, '03_初稿', '.analysis'));

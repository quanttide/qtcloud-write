...
import 'package:qtcloud_write_studio/blocs/analyze/analyze_bloc.dart';
import 'package:qtcloud_write_studio/models/analysis.dart';
import 'package:qtcloud_write_studio/models/chapter.dart';
import 'package:qtcloud_write_studio/repositories/analysis_repository.dart';
import 'package:qtcloud_write_studio/repositories/chapter_repository.dart';
...
class _TempFileRepository implements ChapterRepository {
  ...
  _TempFileRepository({required this.dir, required this.content})
      : _file = File(path.join(dir.path, '03_初稿', '1_1_章节.md')),
        _chapter = Chapter(
          id: '1_1_章节',
          title: '章节',
          stageId: '03_初稿',
          path: path.join(dir.path, '03_初稿', '1_1_章节.md'),
...
void main() {
  ...
  test('分析流程：结果落盘、缓存复用不重复调用 LLM、原文不变', () async {
    ...
    expect(bloc.state.analysis!.suggestedStageId, '04_定稿');
  ...
  test('01_收集：采纳灵感片段 → 创建 02_分组 文件（原样摘录，日志原文不变）', () async {
    final repo = _TempFileRepository(dir: tempDir, content: originalContent);
    final analysisRepo = FileAnalysisRepository();
    final llm = _FakeLLMClient();
    ...
    final analysis = ChapterAnalysis(
      chapterId: '1_1_章节',
      chapterPath: path.join(tempDir.path, '03_初稿', '1_1_章节.md'),
      stageId: '01_收集',
    ...
    expect(chapters.where((c) => c.stageId == '02_分组').length, 1);
    final created = chapters.firstWhere((c) => c.stageId == '02_分组');
